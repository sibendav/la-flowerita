...
module.exports = class Shoppinglist {
    ...
    static async addNewProductToCart(req, res, next){
        ...
        console.log(cart);
        cart.products = await ShoppinglistsService.GetProductsDetails(cart.products);
        } else {
                ...
                if(!ssn.cart.products.find(p => {if (p.id === product._id.toString()) {
                      return true;
                    }
                    return false;
                  }))
                {
                    ...
                    ssn.cart.products.push({id:product._id, quantity: 1, price: product.price, image: product.image, name:product.name});
                } else{
                    console.log("duplicates")
                    ssn.cart.products.map(p => p.id == product._id ? p.quantity = p.quantity + 1 : p.quantity = p.quantity);
                }
                console.log(ssn.cart)
        ...
        try{
            ...
            if (req.user) {
              ...
              cart.products = await ShoppinglistsService.GetProductsDetails(cart.products);
                ...
                console.log(req.session)
                let ssn = req.session;
                ssn.cart.products.map(p => p.id == product.productId ? p.quantity = product.quantity : p.quantity = product.quantity);
                req.session.cart = ssn.cart;
                req.session.save((err) => {
                    console.log(err);
                  });
        ...
        try{
            ...
            //   console.log("cart id " + cartId);
              ...
              cart.products = await ShoppinglistsService.GetProductsDetails(cart.products);
                ...
                console.log("product id: " + productId)
                var ssn = req.session;
                ssn.cart.products = ssn.cart.products.filter(p => p.id != productId);
                req.session.cart = ssn.cart;
                req.session.save((err) => {
                    console.log(err);
                  });
